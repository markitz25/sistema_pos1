...
def _dt_inicio(fecha_date):
    """Convierte una fecha a datetime consciente de zona al inicio del d?a."""
    dt = datetime.combine(fecha_date, datetime.min.time())
    return timezone.make_aware(dt) if timezone.is_naive(dt) else dt


def _dt_fin(fecha_date):
    """Convierte una fecha a datetime consciente de zona al fin del d?a."""
    dt = datetime.combine(fecha_date, datetime.max.time())
    return timezone.make_aware(dt) if timezone.is_naive(dt) else dt
...
def dashboard_admin(request):
    ...
    inicio_mes_dt = _dt_inicio(inicio_mes)
    inicio_semana_dt = _dt_inicio(inicio_semana)
...
def api_ventas_por_mes(request):
    ...
    for i in range(11, -1, -1):
        ...
        if i > 0:
            ...
            fin_mes = hoy
        fin_mes_dt = _dt_fin(fin_mes)
        
        # Calcular total del mes
        total = Venta.objects.filter(
            fecha__gte=inicio_mes_dt,
            fecha__lte=fin_mes_dt,
            estado='completada'
