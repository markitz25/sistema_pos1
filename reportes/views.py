...
def dashboard(request):
    ...
    # Ventas del mes (para cards)
    ventas_mes = Venta.objects.filter(
        fecha__gte=inicio_mes,
        estado='completada'
    )
    total_ventas = ventas_mes.aggregate(total=Sum('total'))['total'] or Decimal('0')
    cantidad_ventas = ventas_mes.count()
    promedio_venta = ventas_mes.aggregate(prom=Avg('total'))['prom'] or Decimal('0')
    ...
    # Cliente con más compras acumuladas
